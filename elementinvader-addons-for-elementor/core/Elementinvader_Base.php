...
class Elementinvader_Base extends Widget_Base {
	...
	protected function register_controls() {
            ...
            $this->add_control(
                'suffix_view_file',
                [
                        'label'         => esc_html__( 'Suffix for view file', 'elementinvader-addons-for-elementor' ),
                        'description'         => esc_html__( 'Using only for custom view layouts, added like suffix example widget.php to widget{your-suffix}.php', 'elementinvader-addons-for-elementor' ),
	...
	protected function render() {
        ...
        public function view($view_file = '', $element = NULL, $print = false)
        {
            ...
            $view_file_org = $view_file;

            if(!empty($settings['suffix_view_file'])) {
                $view_file .= sanitize_file_name($settings['suffix_view_file']);
            }
            ...
            elseif(is_child_theme() && file_exists(get_stylesheet_directory().'/elementor-elementinvader_addons_for_elementor/views/'.$this->view_folder.'/'.($view_file).'.php'))
            {
                $file = get_stylesheet_directory().'/elementor-elementinvader_addons_for_elementor/views/'.$this->view_folder.'/'.($view_file).'.php';
            }
