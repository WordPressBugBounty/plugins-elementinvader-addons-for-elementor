...
<div class="elementinvader_addons_for_elementor_f_group recaptcha elementinvader_addons_for_elementor_f_group_el_<?php echo esc_attr($element['_id']);?>">
    ...
    echo '<div id="recaptcha_called_'.$this->get_id_int().'" class="g-recaptcha" '.$compact_tag.' data-sitekey="'.esc_attr($settings['recaptcha_site_key']).'"></div>';
    ?>

    <script>
    var CaptchaCallback_<?php echo $this->get_id_int();?> = function(){
         //   grecaptcha.render(document.getElementById('recaptcha_called_<?php echo $this->get_id_int();?>'), {'size' : '',  'sitekey' : '<?php echo esc_attr($settings['recaptcha_site_key']);?>'});
   };
    </script>
</div>